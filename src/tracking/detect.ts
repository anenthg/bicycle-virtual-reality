...
/**
 * One pass over the frame, testing every pixel against both HSV windows and
 * accumulating centroid sums. ~77k pixels at 320x240 — comfortably inside the
 * 12 ms worker budget in plain JS.
 */
export function detectMarkers(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++, i += 4) {
      ...
      if (s < left.satMin && s < right.satMin) continue;
